...
import lasagne
# For downloading the sample text file. You won't need this if you are
...
def main(num_epochs=NUM_EPOCHS):
    ...
    # The output of the sliced layer will then be of size (batch_size,
    # N_HIDDEN)
    l_forward_slice = lasagne.layers.SliceLayer(l_forward_2, -1, 1)

    # The sliced output is then passed through the softmax nonlinearity to create probability distribution of the prediction
    # The output of this stage is (batch_size, vocab_size)
    l_out = lasagne.layers.DenseLayer(l_forward_slice, num_units=vocab_size, W=lasagne.init.Normal(
    ...
    cost = T.nnet.categorical_crossentropy(
        network_output, target_values).mean()
    ...
    compute_cost = theano.function(
        [l_in.input_var, target_values], cost, allow_input_downcast=True)

    # In order to generate text from the network, we need the probability distribution of the next character given
    # the state of the network and the input (a seed).
    # In order to produce the probability distribution of the prediction, we
    # compile a function called probs.
    ...
    # The optional input "N" is used to set the number of characters of text
    # to predict.
    ...
    def try_it_out(N=200):
        ...
        sample_ix = []
        x, _ = gen_data(len(generation_phrase) -
                        SEQ_LENGTH, 1, generation_phrase, 0)
        ...
        random_snippet = generation_phrase + \
    ...
    try:
        for it in xrange(data_size * num_epochs / BATCH_SIZE):
            # Generate text using the p^th character as the start.
            try_it_out()
            ...
            print("Epoch {} average loss = {}".format(
                it * 1.0 * PRINT_FREQ / data_size * BATCH_SIZE, avg_cost / PRINT_FREQ))
