################################
# Shamelessly taken form Felix Laumon
#  https://github.com/felixlaumon/nolearn_utils
################################
...
class PlotTrainingHistory(object):
    ...
    def __call__(self, nn, train_history):
        ...
        valid_accuracy = np.asarray(
            [history['valid_accuracy'] for history in train_history])
        train_loss = np.asarray([history['train_loss']
                                 for history in train_history])
        valid_loss = np.asarray([history['valid_loss']
                                 for history in train_history])
